...
@router.get("/")
def read_root(request: Request, _=Depends(require_auth)):
...
@router.get("/calendar")
def read_calendar(request: Request, _=Depends(require_auth)):
    return templates.TemplateResponse(request=request, name="calendar.html", context={"request": request})

@router.get("/patients")
def read_patients(request: Request, _=Depends(require_auth)):
...
async def interns_page(request: Request, _=Depends(require_auth)):
...
async def fitting_list_page(request: Request, _=Depends(require_auth)):
...
async def reports_absences_page(request: Request, _=Depends(require_auth)):
...
async def reports_weekly_page(request: Request, _=Depends(require_auth)):
...
async def waiting_list_page(request: Request, _=Depends(require_auth)):
...
async def reports_waiting_list_page(request: Request, _=Depends(require_auth)):
...
async def reports_attendance_page(request: Request, _=Depends(require_auth)):
...
async def reports_patient_attendance_page(request: Request, _=Depends(require_auth)):
